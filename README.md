...
- Passwordless-first customer access through one Supabase email Magic Link flow for new and returning customers, with persisted sessions, safe callback return paths, logout, and authenticated API bearer tokens. Email/password login and password sign-up remain accessible secondary paths and follow Supabase email-confirmation policy.
- Quiet first-login profile discovery never blocks the website while loading. Incomplete customers may save name/phone and an optional first vehicle or persistently choose **Skip for now**; authenticated booking completion also updates reusable name/phone details and safely saves new vehicles once by normalized plate.
...
- Verified first-party reviews are bound to completed bookings with one review per booking, 1–5 stars, optional plain-text comments, abbreviated public names, and no public contact/location/vehicle PII. Logged-in customers use profile ownership; guests use a management token or a short-lived device-bound proof after booking-reference/phone verification.
- The homepage highlights actual top-rated commented reviews, while `/reviews` presents all published feedback without suppressing legitimate lower ratings. Eligible logged-in customers receive a small server-paced prompt once every one to three genuine browser sessions, shared across their devices.
- Customer Profile includes a deliberate self-service account-deletion flow. FastAPI deletes the Supabase Auth identity server-side, removes reusable profile/address/vehicle/loyalty/review state, blocks stale deleted identities from reprovisioning, and retains anonymized booking plus immutable financial snapshots where required.
...
- Manager/admin Inventory area with a simple business-level **Automatic consumables stock** setting, controlled consumable catalogue, future-ready main/team/van locations, low/out alerts, bounded search, append-only movements, receiving, transfers, usage, wastage, optional physical stock counts, and optional receipt-to-expense recording. A configured default, sole active location, or sole obvious Main Shop powers immediate completion-time deductions; routine washes require neither manager approval nor a stock count. Genuine exceptions use plain-language, issue-specific actions, and Mark Reviewed removes the active card without changing history.
...
- Manager/admin Services & Pricing area for mobile-only startup catalogue CRUD, activate/deactivate, per-vehicle prices, expected duration, optional add-ons, weekday operating hours, controlled public 60/90/120-minute slot settings, cancellation cutoff, mobile minimum, loyalty reward-service selection, and expected-consumables templates. Shop compatibility fields remain backend-only and are absent from the normal manager/customer UI. Employees may read operational catalogue/settings data but cannot mutate it.
...
- Completed-booking review ownership, database uniqueness/rating constraints, bounded guest proof attempts, server-side prompt cadence, public PII-safe serializers, and deleted-identity tombstones are enforced by FastAPI/PostgreSQL rather than browser state.
