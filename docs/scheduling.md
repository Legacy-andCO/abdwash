...
All current Trifecta operational appointment presentation uses the IANA timezone `Asia/Dubai`, independently of the employee device, browser, Vercel process, or email provider timezone. API/database timestamps remain timezone-aware UTC instants. For example, a manager-selected `2026-08-31 10:00` wall-clock time is interpreted as UAE time, stored as `2026-08-31T06:00:00Z`, and rendered as `10:00` by mobile, web, calendar, and email consumers. Mobile date filters and “Today” also use the UAE calendar day rather than `toISOString()`'s UTC day.
...
The exact-reschedule path emits safe stage timings from booking/job locks through day locking, scheduler selection, slot replacement, reminder cleanup, projections, sync revisions, and commit. No customer fields or provider credentials are logged. A mobile timeout is treated as an uncertain write: the client retains the same event ID, performs one authoritative Job read, accepts the operation as successful when that schedule already matches, and otherwise offers a retry using the same logical event.
