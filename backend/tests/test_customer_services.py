...
from app.services.customers import map_customer_status, reschedule_customer_booking
...
def test_customer_status_mapping_prioritizes_cancellation() -> None:
    requested = map_customer_status(
        BookingStatus.CANCELLATION_REQUESTED, JobStatus.ASSIGNED
    )
...
async def test_guest_booking_never_claims_profile_by_email() -> None:
    ...
    assert await _resolve_customer_profile(
        session,
        identity=None,
        business_id=uuid.uuid4(),
        contact=contact,
...
async def test_reschedule_atomically_swaps_slots_and_updates_job() -> None:
    ...
    assert any(
        isinstance(call.args[0], JobEvent)
        and call.args[0].event_type == "booking_rescheduled"
