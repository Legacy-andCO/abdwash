...
const job = (id: string, status: string): Job => ({
  id, status, booking_id: id, booking_reference: id, assigned_staff_id: null,
  assigned_staff_name: null, assigned_team_id: null, assigned_team_name: null,
  scheduled_start: "2026-08-24T09:00:00Z", scheduled_end: "2026-08-24T11:00:00Z",
  en_route_at: null, estimated_arrival_at: null, started_at: null, completed_at: null,
  customer_name: "Customer", customer_phone: "+971501234567", written_address: "Abu Dhabi",
  location_url: "https://www.google.com/maps", latitude: null, longitude: null,
  location_instructions: null, payment_status: "unpaid", payment_method: null,
  total_amount_minor: 10000, currency_code: "AED", vehicles: [],
});
...
describe("operations navigation and permissions", () => {
  ...
  it.each(["manager", "admin"] as const)("gives %s five management tabs", (role) => {
    expect(navigationTabs(role)).toEqual(["today", "jobs", "team", "reports", "profile"]);
  });
...
describe("authoritative operation responses", () => {
  ...
  it("chooses the next incomplete employee job", () => {
    expect(nextOperationalJob([job("one", "completed"), job("two", "assigned")])?.id).toBe("two");
  });
  it.each(["en_route", "in_progress"])("requires confirmation for %s reassignment", (status) => {
...
describe("attendance, reports and resilient states", () => {
  it("calculates a client display timer without persisting ticks", () => {
    expect(attendanceElapsedMinutes("2026-08-24T09:00:00Z", Date.parse("2026-08-24T10:15:00Z"))).toBe(75);
  });
  ...
  it("uses aggregated report values", () => {
    const points = [{ booked_sales_minor: 200 }, { booked_sales_minor: 500 }] as ReportPoint[];
  ...
  it("turns server conflict codes into useful messages", () => {
    expect(conflictMessage("TEAM_ASSIGNMENT_CONFLICT")).toContain("already has work");
