...
export function HomeReviews() {
  ...
  return <ScrollReveal as="section" className="section home-reviews">
    <div className="shell">
      ...
      {summary.featured_reviews.length ? <ScrollReveal className="review-card-grid" stagger>{summary.featured_reviews.map((review) => <ReviewCard key={review.id} review={review} />)}</ScrollReveal> : <p className="review-empty">{t("reviews.empty")}</p>}
