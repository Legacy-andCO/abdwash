...
import { ApiError, createBooking, createHold, friendlyError, getAvailability, getCatalogue, getCustomerProfile } from "@/lib/api";
...
function WizardProgress({ step }: { step: BookingStep }) {
  ...
  return (
    <div className="wizard-progress">
      <p>Step {Math.min(active + 1, 6)} of 6</p>
    ...
    <header className="booking-header">
      <div className="shell">
        ...
        <Link className="quiet-link" href="/">
          Save & exit <span aria-hidden="true">×</span>
        </Link>
      </div>
    </header>
  );
...
export function BookingWizard({ initialServiceId }: { initialServiceId: string }) {
  ...
  const loadCatalogue = () => {
    ...
    getCatalogue()
      ...
      .catch((error) => setCatalogueError(friendlyError(error)))
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([getCatalogue(), user ? getCustomerProfile() : Promise.resolve(null)]).then(([catalogueResult, profileResult]) => {
      if (!active) return;
      if (catalogueResult.status === "rejected") setCatalogueError(friendlyError(catalogueResult.reason));
      else {
      ...
      else if (profileResult.status === "rejected") setProfileWarning("We couldn’t load your saved details, but you can still book normally.");
      setLoading(false);
    });
    return () => {
      active = false;
        ...
        <div className="shell booking-layout">
          <aside>
            ...
            <div className="booking-aside-note">
              ...
              <p>Your choices stay here while you move between steps.</p>
            </div>
          </aside>
          <section className="wizard-card" aria-live="polite">
            ...
            {loading ? <LoadingPanel /> : catalogueError ? <ErrorPanel message={catalogueError} onRetry={loadCatalogue} /> : state.catalogue ? <WizardStep state={state} dispatch={dispatch} /> : null}
          </section>
        </div>
      </main>
    </>
...
function StepActions({ back, next, nextLabel = "Continue", busy = false, disabled = false }: { back?: () => void; next: () => void; nextLabel?: string; busy?: boolean; disabled?: boolean }) {
  return (
...
function ServiceStep({ state, dispatch }: StepProps) {
  ...
  return (
    <>
      <StepIntro eyebrow="Choose your care" title="What does your car need?" copy="Select a starting service. You can assign a different service to each vehicle later." />
        ...
        {services.map((service) => (
          <label className={state.defaultServiceId === service.id ? "choice-card selected" : "choice-card"} key={service.id}>
            ...
            <span className="choice-content">
              ...
              <small>{service.description ?? "Professional mobile car care at your location."}</small>
              <em>≈ {service.estimated_duration_minutes} min</em>
            </span>
            <b>{formatMoney(service.price_minor, service.currency_code)}</b>
          </label>
        ))}
      </div>
...
function DetailsStep({ state, dispatch }: StepProps) {
  ...
  return (
    <>
      <StepIntro eyebrow="About you" title="Where should we meet you?" copy="We’ll use these details only to arrange and confirm your service." />
      {savedLocations.length > 0 && (
        <div className="form-section">
          <h2>Use saved location</h2>
      ...
      <div className="form-section">
        ...
        <div className="form-grid two">
          {contactField("first_name", "First name", "text", "given-name")}
          {contactField("surname", "Surname", "text", "family-name")}
          {contactField("email", "Email address", "email", "email")}
...
function VehiclesStep({ state, dispatch }: StepProps) {
  const [errors, setErrors] = useState<Record<string, string>>({});
  const next = () => {
    const found = vehicleErrors(state.vehicles);
    setErrors(found);
  ...
  return (
    <>
      <StepIntro eyebrow="Your vehicles" title={state.vehicles.length === 1 ? "Tell us about your car." : `Tell us about your ${state.vehicles.length} cars.`} copy="Add every vehicle in this visit and choose the right service for each." />
            ...
            {state.customerProfile!.vehicles.map((vehicle) => (
              <button className="choice-card" type="button" key={vehicle.id} onClick={() => dispatch({ type: "saved_vehicle", value: vehicle })}>
                <span className="choice-content">
                  ...
                  <small>{vehicle.plate_number ? `Plate: ${vehicle.plate_number}` : "Saved vehicle"}</small>
                </span>
                <b>Use this vehicle</b>
              </button>
            ))}
          </div>
        </div>
      ...
      {state.vehicles.length >= state.catalogue!.settings.multi_vehicle_threshold && (
        <div className="inline-notice">
          <strong>More time, reserved automatically.</strong>
          <span>The scheduling service will find a consecutive window for this visit.</span>
        </div>
      )}
...
function VehicleCard({ vehicle, index, services, errors, canRemove, dispatch }: { vehicle: Vehicle; index: number; services: Service[]; errors: Record<string, string>; canRemove: boolean; dispatch: StepProps["dispatch"] }) {
  ...
  const input = (field: keyof Vehicle, label: string, placeholder = "") => (
    <label>
      <span>{label}</span>
      <input value={vehicle[field]} placeholder={placeholder} aria-invalid={!!errors[`${vehicle.key}.${field}`]} aria-describedby={errors[`${vehicle.key}.${field}`] ? `${vehicle.key}-${field}-error` : undefined} onChange={(event) => update(field, event.target.value)} />
  ...
  return (
    <fieldset className="vehicle-card">
      ...
      <div className="form-grid two">
        ...
        {input("model", "Model", "Camry")}
        {input("year", "Year (optional)", "2024")}
        <label>
        ...
        {input("colour", "Colour (optional)")}
        {input("plate_number", "Plate number (optional)")}
      </div>
      <label>
        <span>Service for this vehicle</span>
        <select value={vehicle.service_id} aria-invalid={!!errors[`${vehicle.key}.service_id`]} onChange={(event) => update("service_id", event.target.value)}>
          {services.map((service) => (
            <option key={service.id} value={service.id}>
              {service.name} — {formatMoney(service.price_minor, service.currency_code)}
            </option>
          ))}
        </select>
...
function ReviewStep({ state, dispatch }: StepProps) {
  ...
  return (
    <>
      <StepIntro eyebrow="Review" title="Everything look right?" copy="Check the visit before choosing a date. Final pricing is confirmed by the server when you book." />
      <div className="review-sections">
        <ReviewBlock title="Contact" action={() => dispatch({ type: "step", value: "details" })}>
          <strong>
        ...
        <ReviewBlock title="Location" action={() => dispatch({ type: "step", value: "details" })}>
          <strong>{state.location.written_address}</strong>
          <a href={state.location.location_url} target="_blank" rel="noreferrer">
            Open map link ↗
          </a>
          {state.location.instructions && <span>{state.location.instructions}</span>}
        </ReviewBlock>
        <ReviewBlock title={`Vehicles · ${state.vehicles.length}`} action={() => dispatch({ type: "step", value: "vehicles" })}>
          {state.vehicles.map((vehicle) => (
            <div className="review-vehicle" key={vehicle.key}>
              ...
              <b>{services.get(vehicle.service_id)?.name}</b>
            </div>
          ))}
        </ReviewBlock>
        <div className="estimate-row">
          <span>
            ...
            <small>Calculated from current catalogue prices</small>
          </span>
          <b>{formatMoney(estimate, state.catalogue!.settings.currency_code)}</b>
        </div>
      </div>
      <StepActions back={() => dispatch({ type: "step", value: "vehicles" })} next={() => dispatch({ type: "step", value: "schedule" })} nextLabel="Choose a time" />
    </>
  );
}
...
function ScheduleStep({ state, dispatch }: StepProps) {
  ...
  const proceed = async () => {
    ...
    try {
      ...
      setError(reason instanceof ApiError && reason.isSchedulingConflict ? "That time was just taken. We refreshed availability so you can choose another." : friendlyError(reason));
  ...
  return (
    <>
      <StepIntro eyebrow="Your schedule" title="When should we come?" copy={`Times shown in ${timezone.replace("_", " ")}. Availability is live and your time is held on the next step.`} />
      <div className="schedule-grid">
        <div className="calendar">
          <header>
            <button type="button" aria-label="Previous month" disabled={visibleMonthKey <= currentMonthKey} onClick={() => moveMonth(-1)}>
              ←
            </button>
            <h2>{monthLabel}</h2>
            ...
            {calendarCells(visibleMonth.year, visibleMonth.monthIndex).map((day, index) =>
              day ? (
                <button
                  ...
                  aria-label={new Intl.DateTimeFormat("en-AE", {
                    weekday: "long",
                    day: "numeric",
                    month: "long",
                    timeZone: "UTC",
        ...
        <div className="times-panel">
          <h2>{state.selectedDate ? "Available starts" : "Choose a date"}</h2>
          {availabilityLoading && (
            <div className="slot-loading">
              <span className="spinner dark" /> Checking live times…
            </div>
          )}
            ...
            <>
              <div className="slot-grid">
                {state.availability.slots.map((slot) => (
                  <SlotButton key={slot.time} slot={slot} selected={state.selectedSlotTime === slot.time} timezone={timezone} onSelect={() => dispatch({ type: "slot", value: slot.time })} />
                ))}
              </div>
              {!state.availability.slots.some((slot) => slot.available) && <p className="empty-copy">No times are available on this date. Please try another day.</p>}
            </>
          )}
          {!state.selectedDate && <p className="empty-copy">Select a date to see real-time availability.</p>}
        </div>
      </div>
      {state.availability?.required_slot_count && state.availability.required_slot_count > 1 ? (
        <div className="inline-notice">
          <strong>A consecutive window is included.</strong>
          <span>
            For {state.vehicles.length} vehicles, the selected start reserves {state.availability.required_slot_count} back-to-back slots. The end time comes directly from live availability.
          </span>
        </div>
      ) : null}
      {error && (
      ...
      <StepActions back={() => dispatch({ type: "step", value: "review" })} next={proceed} nextLabel="Reserve this time" busy={holding} disabled={!state.selectedSlotTime} />
    </>
  );
}
...
function SlotButton({ slot, selected, timezone, onSelect }: { slot: AvailabilitySlot; selected: boolean; timezone: string; onSelect: () => void }) {
  ...
  return (
    <button type="button" className={selected ? "slot selected" : "slot"} disabled={!slot.available} aria-pressed={selected} title={slot.available ? undefined : slot.unavailable_reason?.replaceAll("_", " ").toLowerCase()} onClick={onSelect}>
      ...
      {slot.required_slot_count > 1 && <small>until {time.format(new Date(slot.ends_at))}</small>}
    </button>
  );
}
...
function PaymentStep({ state, dispatch }: StepProps) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(reason instanceof ApiError && reason.code === "NETWORK_ERROR" ? "The connection was interrupted, so your booking may already be confirmed. Try again to safely recover the same booking." : friendlyError(reason));
  ...
  return (
    <>
      <StepIntro eyebrow="Payment" title="How would you like to pay?" copy="Your slot is temporarily reserved while you confirm." />
      {state.hold && (
        <div className={seconds < 60 ? "hold-timer urgent" : "hold-timer"}>
          <span>Time reserved</span>
          <strong>
        ...
        <div className="choice-list">
          <label className={choice === "pay_after_service" ? "choice-card selected" : "choice-card"}>
            ...
            <span className="choice-content">
              ...
              <small>Pay the team once your wash is complete.</small>
              <em>Available now</em>
            </span>
          </label>
          <label className={choice === "pay_now" ? "choice-card selected" : "choice-card"}>
            ...
            <span className="choice-content">
              ...
              <small>Online card payments are not connected yet.</small>
              <em>Coming soon</em>
            </span>
          </label>
          {choice === "pay_now" && (
            <div className="inline-notice warning" role="status">
              ...
              <span>Choose “Pay after service” to complete this booking. No card details are collected.</span>
            </div>
          )}
        </div>
        <aside className="payment-summary">
          <span>Booking estimate</span>
          <strong>{formatMoney(estimate, state.catalogue!.settings.currency_code)}</strong>
          <p>
            {state.vehicles.length} {state.vehicles.length === 1 ? "vehicle" : "vehicles"}
          </p>
          {state.hold && <small>{formatSchedule(state.hold.starts_at, state.hold.ends_at, state.catalogue!.settings.timezone)}</small>}
        </aside>
      </div>
      {error && (
        <div className="error-banner" role="alert">
      ...
      <StepActions back={() => dispatch({ type: "step", value: "schedule" })} next={submit} nextLabel="Confirm booking" busy={submitting} disabled={choice !== "pay_after_service" || seconds <= 0} />
    </>
  );
}
  ...
  return (
    <main className="confirmation-page">
      ...
      <section className="confirmation-card">
        ...
        <h1>We’ll see you there, {booking.customer_first_name}.</h1>
        <p className="confirmation-lead">Your wash is booked. Keep the reference below handy.</p>
        ...
        <div className="confirmation-grid">
          <div>
            <span>Date & time</span>
            <strong>{formatSchedule(booking.scheduled_start, booking.scheduled_end, state.catalogue!.settings.timezone)}</strong>
          </div>
          <div>
            <span>Payment</span>
            <strong>Pay after service · {booking.payment_status}</strong>
          </div>
          <div>
            <span>Location</span>
          ...
          <div>
            ...
            <strong>{formatMoney(booking.total_amount_minor, booking.currency_code)}</strong>
          </div>
        </div>
        <div className="confirmed-vehicles">
          <h2>{booking.vehicles.length === 1 ? "Vehicle & service" : "Vehicles & services"}</h2>
        ...
        <p className="confirmation-note">A confirmation notification has been queued for {state.contact.email}.</p>
      </section>
    </main>
  );
}
...
function LoadingPanel() {
  return (
    <div className="loading-panel">
      ...
      <p>Loading live services and business settings…</p>
    </div>
  );
}
function ErrorPanel({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="loading-panel">
      <strong>We couldn’t load booking details.</strong>
      <p>{message}</p>
