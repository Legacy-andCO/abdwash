...
function WizardProgress({ step }: { step: BookingStep }) {
  ...
  const stepLabels: Record<BookingStep, string> = {
    service: t("booking.steps.service"),
    details: t("booking.steps.details"),
    vehicles: t("booking.steps.vehicles"),
    review: t("booking.steps.review"),
    schedule: t("booking.steps.schedule"),
  ...
  return (
    <div className="wizard-progress">
      <p>
        {t("booking.progressStep", {
          current: Math.min(active + 1, 6),
          total: 6,
        })}
      </p>
      <div className="progress-track" aria-hidden="true">
        <span
          style={{ width: `${Math.min(((active + 1) / 6) * 100, 100)}%` }}
        />
      </div>
      <ol>
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      if (catalogueResult.status === "rejected")
        setCatalogueError(catalogueResult.reason);
      else {
      ...
      if (profileResult.status === "fulfilled" && profileResult.value)
        dispatch({ type: "customer_bootstrap", value: profileResult.value });
      else if (profileResult.status === "rejected")
        setProfileWarning(translationRef.current("booking.profileWarning"));
      setLoading(false);
    });
    return () => {
      active = false;
        ...
        <div className="shell booking-layout">
          ...
          <section className="wizard-card" aria-live="polite">
            ...
            {loading ? (
              ...
              <ErrorPanel
                message={localizedCustomerError(catalogueError, language, t)}
                onRetry={loadCatalogue}
              />
            ) : state.catalogue ? (
              <WizardStep state={state} dispatch={dispatch} />
            ) : null}
          </section>
        </div>
      </main>
    ...
    <div className="step-actions">
      {back && (
        <button className="button button-ghost" type="button" onClick={back}>
          <span className="directional-icon" aria-hidden="true">
            ←
          </span>{" "}
          {t("common.back")}
        </button>
...
function ServiceStep({ state, dispatch }: StepProps) {
  ...
  return (
    <>
      <StepIntro
        eyebrow={t("booking.service.eyebrow")}
...
function DetailsStep({ state, dispatch }: StepProps) {
  ...
  const contactField = (
    ...
    <label>
      ...
      <input
        ...
        onChange={(event) =>
          dispatch({ type: "contact", field, value: event.target.value })
        }
      />
  ...
  const updateLocationField = useCallback(
    (
      ...
      if (field === "location_url")
        dispatch({ type: "manual_location_url", value });
  ...
  return (
    <>
      <StepIntro
        eyebrow={t("booking.details.eyebrow")}
        title={t("booking.details.title")}
        copy={t("booking.details.copy")}
      />
          ...
          <div className="choice-list compact">
            {savedLocations.map((address) => (
              <button
                ...
                key={address.id}
                onClick={() =>
                  dispatch({ type: "saved_location", value: address })
                }
              >
                <span className="choice-content">
                  <strong>
      ...
      <div className="form-section">
        ...
        <div className="form-grid two">
          ...
          <PhoneInput
            ...
            error={errors.phone}
            onChange={(value) =>
              dispatch({ type: "contact", field: "phone", value })
            }
            onCountryChange={(value) =>
              dispatch({ type: "contact", field: "phone_country", value })
            }
          />
        </div>
      </div>
...
function VehiclesStep({ state, dispatch }: StepProps) {
  ...
  return (
    <>
      <StepIntro
        eyebrow={t("booking.vehicles.eyebrow")}
        title={
          state.vehicles.length === 1
            ? t("booking.vehicles.titleOne")
            : t("booking.vehicles.titleMany", { count: state.vehicles.length })
        }
...
function VehicleCard({
  ...
  const input = (
    ...
    <label>
      <span>{label}</span>
      <input
        required={field === "plate_number"}
        value={vehicle[field]}
        placeholder={placeholder}
        aria-invalid={!!errors[`${vehicle.key}.${field}`]}
        aria-describedby={
          errors[`${vehicle.key}.${field}`]
            ? `${vehicle.key}-${field}-error`
            : undefined
        }
        onChange={(event) => update(field, event.target.value)}
      />
      <FieldError id={`${vehicle.key}-${field}-error`}>
        {errors[`${vehicle.key}.${field}`]}
      </FieldError>
    </label>
  );
      ...
      <div className="form-grid two">
        ...
        <label>
          ...
          <select
            ...
            aria-invalid={!!errors[`${vehicle.key}.vehicle_type`]}
          ...
          <FieldError id={`${vehicle.key}-vehicle_type-error`}>
            {errors[`${vehicle.key}.vehicle_type`]}
          </FieldError>
        </label>
...
function ReviewStep({ state, dispatch }: StepProps) {
  ...
  return (
    <>
      <StepIntro
        eyebrow={t("booking.review.eyebrow")}
      ...
      <div className="review-sections">
        <ReviewBlock
          ...
          action={() => dispatch({ type: "step", value: "details" })}
        >
          <strong>
        ...
        <ReviewBlock
          title={t("booking.review.location")}
          action={() => dispatch({ type: "step", value: "details" })}
        >
          <strong>{state.location.written_address}</strong>
          <a
            href={state.location.location_url}
            target="_blank"
            rel="noreferrer"
          >
          ...
          <span>
            {t("booking.review.locationNotes", {
              notes: state.location.instructions,
            })}
          </span>
        </ReviewBlock>
        <ReviewBlock
          title={`${t("common.vehicles")} · ${state.vehicles.length}`}
          action={() => dispatch({ type: "step", value: "vehicles" })}
        >
          {state.vehicles.map((vehicle) => (
            <div className="review-vehicle" key={vehicle.key}>
              <span>
                ...
                <small>
                  {[vehicle.year, vehicle.colour].filter(Boolean).join(" · ")}
                </small>
                <small className="bidi-ltr">
                  {t("booking.vehicles.plate", { plate: vehicle.plate_number })}
                </small>
              </span>
              <b>
                {localizeServiceName(
                  language,
                  services.get(vehicle.service_id)?.name ?? "",
                )}
              </b>
              {vehicle.loyalty_reward_id ? (
...
function ScheduleStep({ state, dispatch }: StepProps) {
  ...
  const proceed = async () => {
    const slot = state.availability?.slots.find(
      (item) => item.time === state.selectedSlotTime,
    );
    if (!slot) return;
    setHolding(true);
    setError("");
    try {
      ...
      setError(
        reason instanceof ApiError && reason.isSchedulingConflict
          ? t("booking.schedule.taken")
          : localizedCustomerError(reason, language, t),
      );
  ...
  const moveMonth = (delta: number) => {
    const date = new Date(
      Date.UTC(visibleMonth.year, visibleMonth.monthIndex + delta, 1),
    );
    setVisibleMonth({
  ...
  return (
    <>
      <StepIntro
        eyebrow={t("booking.schedule.eyebrow")}
        title={t("booking.schedule.title")}
        copy={t("booking.schedule.copy", {
      ...
      <div className="schedule-grid">
        <div className="calendar">
          <header>
            <button
              ...
              type="button"
              aria-label={t("booking.schedule.previousMonth")}
            ...
            <button
              className="direction-aware"
              type="button"
              aria-label={t("booking.schedule.nextMonth")}
              onClick={() => moveMonth(1)}
            >
              →
            </button>
          ...
          <div className="calendar-grid">
            {calendarCells(visibleMonth.year, visibleMonth.monthIndex).map(
              (day, index) =>
                day ? (
                  <button
                    type="button"
                    ...
                    className={
                      state.selectedDate ===
                      dateKey(visibleMonth.year, visibleMonth.monthIndex, day)
                        ? "selected"
                        : ""
                    }
                    disabled={
        ...
        <div className="times-panel">
          <h2>
            {state.selectedDate
              ? t("booking.schedule.availableStarts")
              : t("booking.schedule.chooseDate")}
          </h2>
          {availabilityLoading && (
              ...
              <div className="slot-grid">
                {state.availability.slots.map((slot) => (
                  <SlotButton
                    key={slot.time}
                    slot={slot}
                    selected={state.selectedSlotTime === slot.time}
                    timezone={timezone}
                    locale={locale}
                    onSelect={() =>
                      dispatch({ type: "slot", value: slot.time })
                    }
                  />
                ))}
              </div>
              {!state.availability.slots.some((slot) => slot.available) && (
                <p className="empty-copy">{t("booking.schedule.noTimes")}</p>
              )}
            </>
          )}
          {!state.selectedDate && (
      ...
      state.availability.required_slot_count > 1 ? (
        <div className="inline-notice">
          <strong>{t("booking.schedule.consecutive")}</strong>
          <span>
            {t("booking.schedule.consecutiveCopy", {
              vehicles: state.vehicles.length,
              slots: state.availability.required_slot_count,
            })}
          </span>
        </div>
      ) : null}
      ...
      <StepActions
        ...
        busy={holding}
        disabled={!state.selectedSlotTime}
      />
    </>
  );
}
  ...
  return (
    <button
      ...
      title={
        slot.available
          ? undefined
          : slot.unavailable_reason?.replaceAll("_", " ").toLowerCase()
      }
      onClick={onSelect}
    >
      ...
      {slot.required_slot_count > 1 && (
        <small>
          {t("booking.schedule.until", {
            time: time.format(new Date(slot.ends_at)),
          })}
        </small>
      )}
    </button>
...
function PaymentStep({ state, dispatch }: StepProps) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof ApiError && reason.code === "NETWORK_ERROR"
          ? t("booking.payment.network")
          : localizedCustomerError(reason, language, t),
      );
      if (reason instanceof ApiError && reason.isSchedulingConflict)
        dispatch({ type: "step", value: "schedule" });
  ...
  return (
    <>
      <StepIntro
        eyebrow={t("booking.payment.eyebrow")}
        title={t("booking.payment.title")}
        copy={t("booking.payment.copy")}
      />
      {state.hold && (
        ...
        <div className="choice-list">
          ...
          >
            <input
              type="radio"
              name="payment"
              checked={choice === "pay_after_service"}
          ...
          <label
            className={
              choice === "pay_now" ? "choice-card selected" : "choice-card"
            }
          >
            <input
              type="radio"
              name="payment"
              checked={choice === "pay_now"}
              onChange={() => setChoice("pay_now")}
            />
            <span className="choice-check" />
            <span className="choice-content">
  ...
  return (
    <main className="confirmation-page">
      ...
      <section className="confirmation-card">
        ...
        <h1>
          {t("booking.confirmation.greeting", {
        ...
        <div className="confirmation-grid">
          ...
          <div>
            ...
            <strong>
              {t("booking.payment.payAfter")} ·{" "}
              {localizePaymentStatus(language, booking.payment_status)}
            </strong>
          </div>
          <div>
        ...
        <div className="confirmed-vehicles">
          <h2>
            {booking.vehicles.length === 1
              ? t("booking.confirmation.vehicleOne")
              : t("booking.confirmation.vehicleMany")}
          </h2>
